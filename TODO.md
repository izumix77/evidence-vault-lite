...
- [x] `EvidenceStatus` union に `"superseded"` / `"stale"` を追加
- [x] zod スキーマを更新
- [x] `effectiveStatus()` との整合確認（topology 導出 vs 明示指定の優先関係）
...
- [x] `HandoverReport` 型を追加
- [x] zod スキーマを追加
- [x] frontmatter パーサーで `type: "handover"` を認識
...
- [x] `EVReport` 型 / `EVReportKind` 型を追加
- [x] zod スキーマを追加
- [x] frontmatter パーサーで `type: "report"` / `report_kind` を認識
...
- [x] `RiskSignal` 型 / `RiskSignalType` 型を追加
...
- [x] `evlite scan` 時に `type: "handover"` / `type: "report"` を検出して `registry.json` に登録
- [x] `HandoverReport.must_read` の ev_id を依存グラフの参照対象に含める
- [x] `EVReport.required_packs_for_continuation` を参照対象に含める
...
- [x] 逆引きインデックスを `registry.json` から構築
- [x] HandoverReport の `must_read` / EVReport の `required_packs_for_continuation` も対象に含める

### 2-E. validate — --show-orphans ✅

- [x] どの pack / node からも参照されていない node を列挙

### 2-F. validate — --show-depends ✅

- [x] `depends_on` / `related` / `supersedes` の構造をツリー表示
- [x] `--active-only` オプション: superseded でない related のみ表示

### 2-G. validate — --show-cycles ✅

- [x] `depends_on` / `supersedes` の循環を検出
...
- [x] `--output <path>` オプションを追加
- [x] 指定した path に validate の全出力を書き込む
- [x] `--show-*` オプションと自由に組み合わせ可能
...
- [x] `--focus <ev_id>` — 指定 node の impact / depends / chains を一括表示
- [x] `--focus-dir <path>` — フォルダ内全 node の情報を一括表示
- [x] `--output` と組み合わせてファイル出力可能
...
- [x] `evlite handover <name>` コマンドを追加
- [x] Constitution v0.5 §3.2 の frontmatter テンプレートを出力
...
- [x] `evlite report <name> --kind <kind> --stack <stack>` コマンドを追加
- [x] Constitution v0.5 §3.3 の frontmatter テンプレートを出力
...
- [x] registry.json から `type: "handover"` の node を返すエンドポイント

### 4-C. GET /api/reports ✅

- [x] registry.json から `type: "report"` の node を返すエンドポイント
...
- [x] HandoverReport 一覧表示
- [x] 新規 HandoverReport 作成フォーム（`evlite handover` 相当の UI）
- [ ] `must_read` の ev_id オートコンプリート（⬜ Phase 5）

### 5-C. Report タブ ✅

- [x] EVReport 一覧表示（Constitution v0.4 の "Implementation Report 登録 UI" をこちらで実装）
- [x] 新規 EVReport 作成フォーム（report_kind セレクタ付き）
- [ ] `required_packs_for_continuation` の pack 候補サジェスト（⬜ Phase 5）
...
- [x] `@ev-lite/core` の pack.md 生成がこの順序をそのまま採用していることを確認・テスト追加
